...
public class GentlyDownTheStream {
    ...
    public List<String> sortedFruitsException() {
        return fruits.stream().filter(a->!a.startsWith("A")).sorted().collect(Collectors.toList());
    }
    ...
    public List<Integer> topTenUniqueOdd() {
        return integerValues.stream().sorted(Comparator.reverseOrder()).distinct().filter(a -> a % 2 != 0).limit(10).collect(Collectors.toList());
    }
